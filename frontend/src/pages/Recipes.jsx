      ...
      <div className="search-panel">
        <div className="search-row">
          <div className="search-group">
            ...
            <input placeholder="Например: Борщ, Паста..." value={titleQuery} onChange={e => setTitleQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} />
          </div>
          <div className="search-group" style={{ position: 'relative' }}>
            <label>По ингредиентам</label>
            <input ref={inputRef} placeholder="Начните вводить (курица, рис...)" value={ingredientInput} onChange={handleIngredientInput} onBlur={handleBlur} onKeyDown={e => e.key === 'Enter' && suggestions.length > 0 && selectIngredient(suggestions[0])} />
            {showDropdown && suggestions.length > 0 && (
              <ul className="suggestions-list" onMouseDown={e => e.preventDefault()}>
                {suggestions.map(p => (<li key={p.id} onMouseDown={() => selectIngredient(p)}>{p.name}</li>))}
              </ul>
            )}
          </div>
        </div>
